...
import com.android.calculator.calculator.division_by_0
import com.android.calculator.calculator.domain_error
import com.android.calculator.calculator.is_infinity
import com.android.calculator.calculator.parser.Expression
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.slidingLayout?.addPanelSlideListener(object : com.sothree.slidinguppanel.PanelSlideListener {
            ...
            override fun onPanelStateChanged(
                panel: View,
                previousState: com.sothree.slidinguppanel.PanelState,
                newState: com.sothree.slidinguppanel.PanelState
            ) {
                if (newState == com.sothree.slidinguppanel.PanelState.ANCHORED) { // To prevent the panel from getting stuck in the middle
                    binding.slidingLayout?.setPanelState(com.sothree.slidinguppanel.PanelState.EXPANDED)
                }
            }
        })
        ...
        binding.historySlidingLayoutButton?.setOnClickListener {
            if (binding.slidingLayout?.getPanelState() == com.sothree.slidinguppanel.PanelState.EXPANDED) {
                binding.slidingLayout?.setPanelState(com.sothree.slidinguppanel.PanelState.COLLAPSED)
            } else {
                binding.slidingLayout?.setPanelState(com.sothree.slidinguppanel.PanelState.EXPANDED)
            }
        }
    ...
    private fun setupUI() {
        ...
        if (MyPreferences(this).preventPhoneFromSleeping) {
    ...
    override fun onResume() {
        ...
        view.keepScreenOn = MyPreferences(this).preventPhoneFromSleeping
    ...
    private fun roundResult(result: BigDecimal): BigDecimal {
        ...
        if (MyPreferences(this).writeNumberIntoScientificNotation && (newResult >= BigDecimal(9999) || newResult <= BigDecimal(
                0.1
            ))
        ) {
