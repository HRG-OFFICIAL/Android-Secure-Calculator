...
class Themes(private val context: Context) {
    ...
    companion object {
        ...
        fun applyTheme(context: Context) {
            val themes = Themes(context)
            val theme = themes.getTheme()
            (context as Activity).setTheme(theme)
            
            // Apply Material You dynamic colors if available and selected
            if (MyPreferences(context).theme == MATERIAL_YOU_THEME_INDEX && DynamicColors.isDynamicColorAvailable()) {
                DynamicColors.applyToActivityIfAvailable(context as Activity)
            }
        }
        
        fun openThemeSelector(context: Context) {
            openDialogThemeSelector(context)
        }
        
        fun openStyleSelector(context: Context) {
            openDialogThemeSelector(context)
        }
    }
