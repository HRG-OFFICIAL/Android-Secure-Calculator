...
# Build script paths
$CleanBuildScript = "$ProjectRoot\build-clean-apk-test.ps1"
$ProtectedBuildScript = "$ProjectRoot\build-protected-apk-test.ps1"
...
function Test-BuildScripts {
    ...
    if (Test-Path $CleanBuildScript) {
        Write-Host "  Clean build script found: build-clean-apk-test.ps1" -ForegroundColor Green
    } else {
        Write-Host "  Clean build script not found!" -ForegroundColor Red
        Write-Host "     Expected: build-clean-apk-test.ps1" -ForegroundColor Yellow
        $allScriptsFound = $false
    }
    
    # Check for protected build script
    if (Test-Path $ProtectedBuildScript) {
        Write-Host "  Protected build script found: build-protected-apk-test.ps1" -ForegroundColor Green
    } else {
        Write-Host "  Protected build script not found!" -ForegroundColor Red
        Write-Host "     Expected: build-protected-apk-test.ps1" -ForegroundColor Yellow
        $allScriptsFound = $false
    }
